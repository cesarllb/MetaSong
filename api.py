...
async def search_songs(artist: str, album: str, precision: float = 0.9) -> list[str]:
    ...
    result = await musicbrainzngs.search_recordings(artist=artist + f"~{precision}", release=album + f"~{precision}")
    title_list: list = []
...
async def search_artist(artist: str = None, precision: float = 0.9, threshold: int = 6) -> str:
    result = await musicbrainzngs.search_artists(artist=artist + f"~{precision}")
    similar_artist = ''
...
async def search_albums(artist: str, precision: float = 0.9) -> list[str]:
    '''Precision says how much error tolerant the search must be'''
    result = await musicbrainzngs.search_releases(artist=artist + f"~{precision}")
    album_list: list = []
...
async def search_album(artist: str, album: str, precision: float = 0.9, threshold: int = 6) -> str:
    '''Precision says how much error tolerant the search must be'''
    result = await musicbrainzngs.search_releases(artist = artist + f"~{precision}", release = album + f"~{precision}")
    if result["release-list"]:
...
async def search_album_by_song(artist: str, song: str, precision: float = 0.9) -> str:
    '''Precision says how much error tolerant the search must be'''
    result = await musicbrainzngs.search_recordings(artist = artist + f"~{precision}", recording = song + f"~{precision}")
    if result["recording-list"]:
